...
"""
    ...
    new_log_entry = ""
    if LOG_BUFFER:
        new_log_entry = f"\n### 🗓️ {datetime.now().strftime('%Y-%m-%d Execution')}\n"
        new_log_entry += "\n".join(LOG_BUFFER) + "\n"
        new_log_entry += "\n----------------------------------------\n"
...
def ai_correction(user_text, translation_history, progress_status):
    ...
    # 🔥 斯巴達教練 Prompt - v0.0.25 語感糾正與錯誤懲罰分離
    prompt = f"""
    ...
    6. **📊 【逐句評分與教練短評】(v22 核心升級)**：
       ...
       - **⚠️ 注意：以上括號內的短評僅為「語氣範例」，絕對禁止照抄！請根據使用者實際犯的錯誤（如時態、敬語、單字）或是精彩之處，即興創作出「當下最貼切」的毒舌或讚美。請展現你豐富的詞彙量，不要重複。**
       - **評分標準**：
...
def process_data():
    ...
    default_user_data = {
        "stats": {
            ...
            "sprint_start_date": str(datetime.now().date())
        },
        "pending_answers": "",
        "translation_log": []
    }
    ...
    url = f"[https://api.telegram.org/bot](https://api.telegram.org/bot){TG_BOT_TOKEN}/getUpdates"
    ...
    try:
        ...
        for item in response["result"]:
            ...
            found_count += 1
            msg_time = datetime.fromtimestamp(message_obj["date"]).strftime('%H:%M:%S')
...
def run_daily_quiz(vocab, user):
    ...
    word_list_str = ""
    for w in quiz_words:
        mark = "🔥" if w in selected_weaks else ""
        w_type = w.get("type", "word")
        type_str = "(文法)" if w_type == "grammar" else ""
        word_list_str += f"{mark}{w['kanji']} {type_str} ({w['meaning']})\n"
