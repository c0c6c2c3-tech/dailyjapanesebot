...
def ai_correction(user_text, translation_history):
    ...
    【格式嚴格要求】
    1. **語言**：解說與評語請全程使用「繁體中文」(Traditional Chinese)。除了日文例句外，不要使用日文。
    2. **排版**：
...
def process_data():
    ...
    if "last_update_id" not in user_data["stats"]: user_data["stats"]["last_update_id"] = 0
    ...
    try:
        ...
        for item in response["result"]:
            ...
            # 如果這則訊息已經處理過，跳過
            if current_update_id <= last_processed_id:
                continue
            
            if current_update_id > max_id_in_this_run:
                max_id_in_this_run = current_update_id
            ...
            # Case A: JSON 匯入 (即使是 Fresh Start 也允許匯入，方便設定資料)
            if text.startswith("["):
                try:
            ...
            if match:
                # ✅ 如果是 Fresh Start，這可能只是舊聊天記錄，不是使用者現在想存的，跳過
            ...
            if not text.startswith("/"):
                # ✅ 如果是 Fresh Start，直接忽略舊的翻譯練習，避免鬼打牆
                if is_fresh_start: continue
...
def run_daily_quiz(vocab, user):
    ...
    # 2. 判斷情緒 Prompt
    # ✅ 強制天使模式：如果是第 1 次執行，或者重置後的第一次 (Day 1)
